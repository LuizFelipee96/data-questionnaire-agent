...
class Config:
    ...
    question_cache_folder = Path(os.getenv("QUESTION_CACHE_FOLDER")).expanduser().resolve()
    create_if_not_exists(question_cache_folder)

    wkhtmltopdf_binary = Path(os.getenv("WKHTMLTOPDF_BINARY")).expanduser().resolve()
    assert wkhtmltopdf_binary.exists(), f"wkhtmltopdf binary not found at {wkhtmltopdf_binary}"

    template_location = Path(os.getenv("TEMPLATE_LOCATION")).expanduser().resolve()
    print(f"\n✴️ template_location", template_location)
    assert template_location.exists(), f"Template location {template_location} does not exist."

    pdf_folder = Path(os.getenv("PDF_FOLDER")).expanduser().resolve()
    ...
    # Embedding related
    raw_text_folder = Path(os.getenv("RAW_TEXT_FOLDER")).expanduser().resolve()
    embeddings_persistence_dir = Path(os.getenv("EMBEDDINGS_PERSISTENCE_DIR")).expanduser().resolve()
    ...
    # Verifique a existência dos diretórios
    assert raw_text_folder.exists(), f"RAW_TEXT_FOLDER path {raw_text_folder} does not exist."
    assert embeddings_persistence_dir.exists(), f"EMBEDDINGS_PERSISTENCE_DIR path {embeddings_persistence_dir} does not exist."
...
try:
        webbrowser.open(url)
        logger.info(f"Abrindo o navegador com a URL: {url}")
except Exception as e:
        logger.error(f"Erro ao tentar abrir o navegador: {e}")